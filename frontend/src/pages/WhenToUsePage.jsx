  ...
  },
  {
    ...
    flow: [
      ...
      { label: 'CS API: Prompt Shields + Text Analysis + Custom Categories', type: 'check',
        ...
        whyNot: 'CF returns per-category string severity labels ("safe", "low", "medium", "high") — not numeric scores. MiFID II and FINRA audit evidence requires quantifiable data: "violence scored 4/6 on 312 requests this week, up from 1/6 last week". String labels cannot support that trending or statistical analysis. CF is also not callable independently of a model call — there is no way to run CF pre-inference separately from the Azure OpenAI request itself.' },
      { label: 'LLM w/ Content Filters (CF-Demo-ContentSafety: Hate, Violence, Sexual, Self-Harm)', type: 'model',
        why: 'CF is platform-enforced — it fires synchronously during inference and cannot be bypassed by application code. CF-Demo-ContentSafety blocks four harm categories (Hate, Violence, Sexual, Self-Harm) on both input and output. This is the guarantee the app-layer CS API call alone cannot provide: even if a developer ships a bug that skips the CS API call, CF still runs.',
        whyNot: 'CS API alone at the app layer can be inadvertently skipped if code paths diverge. CF is the second failure boundary that cannot be short-circuited. Note: Jailbreak is caught by CS API Prompt Shields pre-inference (Step 2) — not by this CF harm-category guardrail.' },
      { label: 'CS API: Groundedness + PII + Protected Material', type: 'check',
        ...
        whyNot: 'CF output harm categories return string severity labels (safe/low/medium/high) — not the numeric 0-6 scores needed for audit trending. CF PII output filter is block/allow only with no entity enumeration (type, offset, redacted text) — insufficient for a PII audit log or targeted redaction. CF Groundedness requires streaming mode and special document-embedding markup and is available in only 4 regions — CS API Groundedness works on any output text, any region, synchronously.' },
  ...
  },
  {
    ...
    flow: [
      ...
      { label: 'CS API: Prompt Shields (direct jailbreak detection)', type: 'check',
        why: 'CS API Prompt Shields returns a structured per-turn detection result (attackDetected: boolean per user prompt and per document) that your application code receives and can log, act on, or route before the model is called. You control the enforcement logic.',
      ...
      { label: 'Agent LLM w/ CF-Demo-Prompt-Shield (Jailbreak + XPIA blocked at inference)', type: 'model',
        why: 'CF-Demo-Prompt-Shield is attached to the cf-demo-markets-assistant agent. It blocks Jailbreak on UserInput (prevents system prompt override at inference time) and IndirectAttack on ToolResponse (blocks XPIA payloads the agent tries to act on). This enforcement happens at the platform layer — application code cannot bypass it.',
        whyNot: 'CF does NOT screen the raw tool return value before it enters the agent context — it only acts at the model inference boundary. An XPIA payload in an unevaluated tool result is caught by CS API Prompt Shields for Documents (Step 5), not by this guardrail. Task Adherence (Step 6) and Groundedness (Step 7) have no CF equivalent.' },
